...
class TempFileRedirectOutput(Silence) :
    
    def __init__(self, mode = 'w') :
        
        stdoutfd, self.stdoutfname = tempfile.mkstemp()
        stderrfd, self.stderrfname = tempfile.mkstemp()
        Silence.__init__(self, stdout = self.stdoutfname, stderr = self.stderrfname, mode = mode)

    def read(self) :
        with open(self.stdoutfname) as f :
            stdout = f.read()
        with open(self.stderrfname) as f :
            stderr = f.read()
        return stdout, stderr

    def __del__(self) :
        for f in self.stdoutfname, self.stderrfname :
            if os.path.exists(f) :
                os.remove(f)
